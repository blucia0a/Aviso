...
func doDistributionUpdate( which uint, rank float64, failureRate float64, dist *distribution.Distribution ){
  ...
  if failureRate < 0{
    ...
    //scale := ( math.Exp( successRate ) ) - 1.0 /*min = 0*/ + 0.001 /*min = 1/1000*/
  ...
  for {
    ...
    if(  val != 1 && !avisoglobal.BaselineOnly ){
      ...
      for classInd := 0; failureClasses[ classInd ].Dist != nil; classInd++ {
        ...
        /*Once we have contenders, use them 75% of the time and explore 25%*/
    ...
    select {
      ...
      case t := <-avisoglobal.StatsChan:
        ...
        if t.Fsm == "baseline" || t.Fsm == "" {
          ...
          fmt.Fprintf(plot,"%v %v %v %v %v\n",totalruns, baselineFailures, baselineSuccesses, anyFsmFailures, anyFsmSuccesses)
          continue
        ...
        if t.Fsm != "" {
          ...
          for fs := range activeFsms{
            ...
            sig := avisostats.ComputeChiSquare(float64(statsUpdate.Failures),float64(statsUpdate.Successes),float64(baselineFailures),float64(baselineSuccesses))
            if sig > 3.85 && rateFromNums(statsUpdate.Failures,statsUpdate.Successes) < rateFromNums(baselineFailures,baselineSuccesses) {
